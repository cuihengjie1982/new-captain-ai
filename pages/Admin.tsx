      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <div className="space-y-4">
              ...
              <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">视频来源</label>
                  <div className="flex gap-4 mb-2">
                      <label className="flex items-center gap-1 cursor-pointer">
                          <input 
                              type="radio" 
                              name="introSourceType"
                              checked={introVideo?.sourceType === 'link'} 
                      ...
                      <label className="flex items-center gap-1 cursor-pointer">
                          ...
                          />
                          <span className="text-sm text-slate-700">本地导入</span>
                      </label>
                  </div>
                  
                  {introVideo?.sourceType === 'upload' ? (
                       <div className="flex gap-2">
                          <input className="w-full border p-2 rounded text-sm bg-slate-50 text-slate-500" value={introVideo?.url?.startsWith('data:') ? '已选择本地文件' : introVideo?.url || ''} disabled placeholder="点击右侧上传" />
                          <label className="p-2 border rounded bg-slate-50 cursor-pointer hover:bg-slate-100 flex items-center gap-1 whitespace-nowrap">
                              <Upload size={16} />
                      ...
                      <input className="w-full border p-2 rounded text-sm" placeholder="https://..." value={introVideo?.url || ''} onChange={e => setIntroVideo(prev => prev ? {...prev, url: e.target.value} : null)} />
                  )}
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                  <div>
                      ...
                      <input className="w-full border p-2 rounded text-sm" value={introVideo?.publisher || ''} onChange={e => setIntroVideo(prev => prev ? {...prev, publisher: e.target.value} : null)} placeholder="例如: Admin" />
                  </div>
                  <div>
                      <label className="block text-xs font-bold text-slate-500 mb-1">发布时间</label>
                  ...
                  <div>
                      ...
                      <input className="w-full border p-2 rounded text-sm" value={introVideo?.duration || ''} onChange={e => setIntroVideo(prev => prev ? {...prev, duration: e.target.value} : null)} placeholder="00:00" />
                  </div>
              </div>
